...
library(tidyverse)
library(sf)
library(tmap)    # for static and interactive maps- works with ggplot syntax
...
Vector: Points, lines, polygons,(coordinates of points) boundaries are crisp regardless of scale Raster: (continuous over rage) Grid of same sized cells, vales in cells, cell size = resolution (smaller cells, higher resolution)
...
*What are the advantages/disadvantages of raster/vector for each?* Which is best to show on a map for each?  \*For elevation, which would be better for calculating slope?  vector is dependent on contor interval. Cant tell what elevation is in blank spaces. Downside to raster for pure DEM is you can't see whats going on but has nice shading.
...
state <- st_read("cb_2018_us_state_20m/cb_2018_us_state_20m.shp")
...
usa+
  ...
  tm_layout(title = "CAMELS Watersheds",
            title.position = c("right", "TOP")) #all caps forces all the way at top
```
...
tm_fill(col = "elev_mean", n = 10, palette = "Blues")+ #10 diff colors
...
tm_layout(legend.position = c("right", "bottom"), inner.margins = 0.1,
...
```{r}
va_watersheds <- filter(watersheds_info, 
                        grepl(", VA", GAGE_NAME)) #like command f

va_outline <- filter(state, NAME == "Virginia")
